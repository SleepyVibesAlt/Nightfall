...
local NightFallUI = isStudio and script.Parent:WaitForChild("NightFall UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		NightFall:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	NightFallUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(NightFallUI)
	NightFallUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	NightFallUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == NightFallUI.Name and Interface ~= NightFallUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "NightFall-Old"
		end
	end
elseif not isStudio then
...
NightFallUI.Enabled = false
NightFallUI.SmartWindow.Visible = false
NightFallUI.Notifications.Template.Visible = false
NightFallUI.DisplayOrder = 1000000000

local Main : Frame = NightFallUI.SmartWindow
local Dragger = Main.Drag
local dragBar = NightFallUI.Drag
...
local Notifications = NightFallUI.Notifications
...
function NightFall:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function NightFall:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "NightFall UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "NightFall Interface Suite" and Release or "NightFall UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(NightFallUI.MobileSupport, NightFallUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "NightFall/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "NightFall/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".NightFall") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".NightFall"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".NightFall", FoundKey)
						end
						NightFall:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "NightFall Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("NightFall Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("NightFall Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- NightFall.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					NightFall.Options[Flag] = SliderV
				end

				NightFallUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = NightFall.ThemeGradient
					Slider.Main.UIStroke.color.Color = NightFall.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("NightFall Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				NightFallUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = NightFall.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = NightFall.ThemeGradient
				end)

				if Flag then
					NightFall.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("NightFall Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					NightFall.Options[Flag] = BindV
				end

				-- NightFall.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("NightFall Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("NightFall Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- NightFall.Flags[DropdownSettings.Flag] = DropdownSettings
				...
				if Flag then
					NightFall.Options[Flag] = DropdownV
				end
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("NightFall Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					NightFall.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = NightFall:SaveConfig(inputPath)
					if not success then
						NightFall:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = NightFall:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = NightFall:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = NightFall:LoadConfig(selectedConfig)
					if not success then
						NightFall:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = NightFall:SaveConfig(selectedConfig)
					if not success then
						NightFall:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(NightFall.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(NightFall.Folder .. "/settings/autoload.txt")
			...
			if isfile(NightFall.Folder .. "/settings/autoload.txt") then
				local name = readfile(NightFall.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if NightFall.Options[Flag] then
						NightFall.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if NightFall.Options[Flag] and data.value then
						NightFall.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if NightFall.Options[Flag] and data.text and type(data.text) == "string" then
						NightFall.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if NightFall.Options[Flag] and data.value then
						NightFall.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if NightFall.Options[Flag] and data.color then
						NightFall.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "NightFallInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "NightFallInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "NightFallInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						NightFall.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						NightFallUI.ThemeRemote.Value = not NightFallUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						NightFall.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						NightFallUI.ThemeRemote.Value = not NightFallUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						NightFall.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						NightFallUI.ThemeRemote.Value = not NightFallUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				NightFall.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				NightFall.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function NightFall:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = NightFall.Folder .. "/settings/" .. Path .. ".NightFall"

			local data = {
				objects = {}
			}

			for flag, option in next, NightFall.Options do
		...
		function NightFall:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = NightFall.Folder .. "/settings/" .. Path .. ".NightFall"
		...
		function NightFall:LoadAutoloadConfig()
			if isfile(NightFall.Folder .. "/settings/autoload.txt") then
				...
				local success, err = NightFall:LoadConfig(name)
				if not success then
					return NightFall:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function NightFall:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(NightFall.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".NightFall" then
					local pos = file:find(".NightFall", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			NightFallUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			NightFallUI.MobileSupport.Visible = false
	...
	NightFallUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		NightFallUI.MobileSupport.Visible = false
...
if isStudio then
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "NightFall's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = NightFall:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
